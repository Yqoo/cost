  ...
  <div id="tags-view-container" class="tags-view-container">
    ...
    <ul v-show="visible" :style="{left:left+'px',top:top+'px'}" class="contextmenu">
      ...
      <li @click="closeOthersTags">
        <i class="el-icon-error" /> 关闭其他
      </li>
      <li @click="closeAllTags(selectedTag)">
        <i class="el-icon-circle-close" /> 关闭所有
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initTags() {
      ...
      const hasWorkbench = affixTags.some(tag => tag.name === 'Workbench')
      if (!hasWorkbench) {
        affixTags.push({
          fullPath: '/workbench',
          meta: {
      ...
      this.$nextTick(() => {
        for (let i = 0, len = tags.length; i < len; i++) {
          if (tags[i].to.path === this.$route.path) {
            this.$refs.scrollPane.moveToTarget(tags[i])
