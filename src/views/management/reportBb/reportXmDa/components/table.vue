<template>
  <div style="height: 100%">
    ...
    <div class="main">
      ...
      >
        <div class="tableOne" style="height:60%;margin-bottom:20px">
          <div class="title">
            <p style="padding-right: 5px">
              <el-button
                v-if="apis.includes('/bbDossierDataController/add')"
                size="mini"
                @click="tableRowHandler('xmadd')"
              >
                添加
              </el-button>
              <el-button
                v-if="apis.includes('/bbDossierDataController/setDossierDataPid')"
                size="mini"
                @click="tableRowHandler('volumeSet')"
              >
                批量设置上级
              </el-button>
              <el-button
                v-if="apis.includes('/bbDossierDataController/delete')"
                size="mini"
                @click="tableRowHandler('xmdels')"
              >
                删除
              </el-button>
            </p>
          ...
          >
            ...
            <el-table-column label="上级编码" prop="pid" width="120">
              <template slot-scope="scope">
                <Treeselect
            ...
            <el-table-column label="关联档案" align="center" prop="xmDossierId" width="120">
              <template slot-scope="scope">
                <!-- :disabled="scope.row.type" -->
                <!-- <span style="display: inline-block; margin-top: 14px;">{{ scope.row.xmDossierName }}</span> -->
                <el-select
                  v-model="scope.row.xmDossierId"
                  :disabled="scope.row.type"
                  placeholder="请选择"
                  @visible-change="(bool)=>changeGLDossier(bool,scope)"
                >
                  <el-option
        ...
        <div class="tableTwo" style="width:100%;height:36%;">
          <span style="color: #4390de;border-bottom: 2px solid #4390de;line-height:30px;padding:6px">
            量度
          </span>
          <el-table
            ref="ldTable"
...
export default {
  components: {
    ...
    ldDetail: () => import('../dialog/ldDetail'),
    Treeselect
  },
  props: {
    xmId: {
  ...
  methods: {
    ...
    getGLDossier() { // 获取关联档案下拉列表
      this.$api.reportxm.getGLDossier({
        xmId: this.dossierId
      }).then(s => {
        this.options = [...s.data]
      }).catch(e => this.$message.error(e.message))
    },
    changeGLDossier(bool, scope) { // 改变关联档案
      if (!bool && this.table.data.xmDossierId !== scope.row.xmDossierId) {
        this.$api.reportxm.editDossierData(scope.row).then(s => {
          this.getdasjData({ id: this.dossierId })
        }).catch(e => this.$message.error(e.message))
      }
    },
    getHigherupData({ xmId, dossierDataId }) { // 获取上级编码下拉树
    ...
    getdasjData({ id = '', page = 1, size = 200 }) { // 通过报表项目id项目选项档案数据
      ...
      this.$api.reportxm.getDASJ({
        ...
        this.table.total = s.data.page.total
    ...
    setTargetNode(node) { // 树节点点击
      ...
      this.$api.reportxm.getTableById({
        id: node.id
      }).then(s => {
        this.table.data = s.data.map(l => ({
          ...l,
          pid: l.pid || null
        }))
      }).catch(e => this.$message.error(e.message))
    },
    inputValue(val) { // treeSelect值改变事件
      if (Object.values(this.table.row).length > 1) {
    ...
    refreshTable(val) {
      this.disabled = false
      this.getdasjData({ id: val || this.dossierId })
      ...
      const Deal = {
        ...
        dataFormulaShow: () => {
          if (scope.row.sn) {
            ...
            if (scope.row.xmDossierId) {
              this.gl.show = true
              this.gl.row = { ...scope.row }
            } else this.$message.warning('请先选择关联档案')
